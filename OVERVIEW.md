...
| Stereo from MapKit? | **No.** No stereo or multi-view API. Use **two `MKMapView`s**, one per eye. MapKit anchors each camera to the surface (rooftops included) under its look-at point, so shifting each eye's center sideways misaligns the eyes vertically. Instead, both cameras look at the same model center from eye positions ±baseline/2 apart (toe-in), and each picture is warped by a homography back to an ideal parallel eye (see `StereoGeometry.swift`). |
