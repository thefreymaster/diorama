...
How that applies to each thing Mini Cities touches:

| Data | What Mini Cities does with it | Collected? |
|---|---|---|
| Place searches | Typed into the search field and sent to Apple Maps (MapKit's `MKLocalSearchCompleter` and `MKLocalSearch`) to find places. Mini Cities has no server of its own. | No: only Apple receives them. |
...
**Tracking: No.** Mini Cities doesn't link data with other companies' data or share it with data brokers. The privacy manifest (`ios.privacyManifests` in `app.json`) matches: `NSPrivacyTracking` false, no tracking domains, no collected data types.

If Mini Cities ever adds analytics, crash reporting, an account or its own server, these answers change before that version ships.
...
```
Mini Cities privacy policy

Effective September 25, 2026

Mini Cities shows real places as tiny 3D models. It doesn't collect your personal information. There is no account, no advertising, no analytics and no tracking, and Mini Cities has no server of its own.

Maps and search
Maps, 3D imagery and place search come from Apple Maps. When you search, or when Mini Cities shows a map, your iPhone sends the request to Apple. Apple's handling of it is described in the Apple Maps privacy notice (Settings > Privacy & Security > Location Services > About Location Services & Privacy, or apple.com/legal/privacy).

Location
Mini Cities uses your location only when you choose Current location, and only while the app is open. It centers the map on you and follows you as you move. Your location is used on your iPhone; to show the name of the place, your iPhone asks Apple Maps. Mini Cities never uses your location in the background, never stores it off your iPhone and never shares it. You can turn location access off at any time in Settings > Privacy & Security > Location Services > Mini Cities.

Camera
If you turn on Lean to move closer, Mini Cities uses the rear camera to follow how your head moves, so you can lean in toward the city. This happens on your iPhone. Camera images are never recorded, stored or sent. You can turn camera access off at any time in Settings > Privacy & Security > Camera, or turn off Lean to move closer in Mini Cities' settings.

Motion
Mini Cities uses your iPhone's motion sensors to turn the view as you move. This data stays on your iPhone.

Data on your iPhone
Your recent places and settings are saved on your iPhone only. Deleting Mini Cities deletes them.

Children
Mini Cities is suitable for all ages and doesn't knowingly collect information from anyone, including children.
